...
// export default supportService;// services/supportService.js
// const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
...
class SupportService {
  ...
  async getUserTickets(filters?: {
    ...
    const response = await fetch(`${API_BASE_URL}/support/tickets/user?${params}`, {
      headers: await this.getAuthHeaders(),
    });
    return this.handleResponse(response);
  }
  ...
  async getFAQs(category?: string): Promise<FAQ[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/support/faq?${params}`, {
      headers: await this.getAuthHeaders(),
    });
    return this.handleResponse(response);
  }
  ...
  // File Upload (if you implement file upload functionality)
  async uploadAttachment(file: File): Promise<{ url: string; filename: string }> {
    const formData = new FormData();
    formData.append('file', file);
  ...
  // Utility methods for local storage (replacing Supabase)
  async incrementKnowledgeBaseViews(id: number): Promise<void> {
    // This would typically make an API call to increment views
    // For now, we'll just store locally or make a simple API call
    try {
      const response = await fetch(`${API_BASE_URL}/support/knowledge-base/${id}/view`, {
        method: 'POST',
        headers: await this.getAuthHeaders(),
      });
  ...
  async incrementFAQHelpful(id: number): Promise<void> {
    try {
      const response = await fetch(`${API_BASE_URL}/support/faq/${id}/helpful`, {
        method: 'POST',
        headers: await this.getAuthHeaders(),
      });
      await this.handleResponse(response);
